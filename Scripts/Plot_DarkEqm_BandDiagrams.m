...
energies_eqm = cell(4, 1);
energies_reverse = cell(4, 1);

[energies_reverse{1,1},energies_reverse{2,1},energies_reverse{4,1},energies_reverse{3,1}] = dfana.calcEnergies(device{1});
[energies_eqm{1,1},energies_eqm{2,1},energies_eqm{4,1},energies_eqm{3,1}] = dfana.calcEnergies(device{2});

%%
x_values = 1e7*device{2}.x;
figure('Name', 'Dark Eqm Energy Levels', 'Position', [50 50 1800 1300])
tiledlayout(1,2)
...
patch('XData',[x_values(1100) x_values(1200) x_values(1200) x_values(1100)], 'YData',[0 0 -8 -8], 'FaceColor', colors_JV{2},...
    'FaceAlpha', 0.5, 'EdgeColor', 'none')
patch('XData',[x_values(1200) x_values(1700) x_values(1700) x_values(1200)], 'YData',[0 0 -8 -8],  'FaceColor', colors_JV{3},...
    'FaceAlpha', 0.3, 'EdgeColor', 'none')

for i = 1:2
    plot(x_values, energies_eqm{i,1}(end,:), 'Color', colours{i})
end
yline(-5.2, 'Color', 'Black', 'LineWidth', 2, 'LineStyle', '--')
hold off
...
for i = 1:4
    if i <= 2
        plot(x_values, energies_reverse{i,1}(end,:), 'Color', colours{i})
    else
        plot(x_values, energies_reverse{i,1}(end,:), 'Color', colours{i}, 'LineStyle','--')
    end
end
hold off
...
if save_fig == 1
    exportgraphics(ax1, ...
    'C:\Users\ljh3218\OneDrive - Imperial College London\PhD\Davide_OrganicPeroHybrid\FigSxa-C60.png', ...
    'Resolution', 300)
end